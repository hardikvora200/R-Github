...
library(wordcloud)
library(syuzhet)
library(rvest) ###Important libraries 
library(XML)
library(magrittr)
aurl <- "https://www.amazon.in/Redmi-Comet-Blue-32GB-Storage/product-reviews/B07QT569GM/ref=cm_cr_dp_d_show_all_btm?ie=UTF8&reviewerType=all_reviews"
amazon_reviews <- NULL
for (i in 1:30){
...
pos.words=scan("C:\\Users\\sanu\\Downloads\\Desktop\\Documents\\Excelr\\Text Mining\\positive-words.txt", what="character", comment.char=";")	# read-in positive-
words.txt
neg.words=scan("C:\\Users\\sanu\\Downloads\\Desktop\\Documents\\Excelr\\Text Mining\\negative-words.txt", what="character", comment.char=";") 	# read-in negative-
words.txt
...
# CLEANING TWEETS
amazon_reviews=gsub("&amp", "", amazon_reviews)
amazon_reviews = gsub("&amp", "", amazon_reviews)
amazon_reviews = gsub("(RT|via)((?:\\b\\W*@\\w+)+)", "", amazon_reviews)
amazon_reviews = gsub("@\\w+", "", amazon_reviews)
amazon_reviews = gsub("[[:punct:]]", "", amazon_reviews)
amazon_reviews = gsub("[[:digit:]]", "", amazon_reviews)
amazon_reviews = gsub("http\\w+", "", amazon_reviews)
amazon_reviews = gsub("[ \t]{2,}", "", amazon_reviews)
amazon_reviews = gsub("^\\s+|\\s+$", "", amazon_reviews)
amazon_reviews <- iconv(amazon_reviews, "UTF-8", "ASCII", sub="")

####Getting sentiments for each tweet
####Syuzhet breaks emotion into 10 different categories
# Emotions for each tweet using NRC dictionary
emotions <- get_nrc_sentiment(amazon_reviews)
emotions
emo_bar = colSums(emotions)##anger - 341,anticipation-553, disgust-67, fear-39,joy-411,
###sadness=73, surprise-250, trust-682,negative-410,positive-1061
...
p <- plot_ly(emo_sum, x=~emotion, y=~count, type="bar", color=~emotion) %>%
  layout(xaxis=list(title=""), showlegend=FALSE,
         title="Emotion Type for hashtag: amazon_reviews")
api_create(p,filename="Sentimentanalysis")
####Lets see which word contributes which emotion
# Create comparison word cloud data
wordcloud_tweet = c(
  paste(amazon_reviews[emotions$anger > 0], collapse=" "),
  paste(amazon_reviews[emotions$anticipation > 0], collapse=" "),
  paste(amazon_reviews[emotions$disgust > 0], collapse=" "),
  paste(amazon_reviews[emotions$fear > 0], collapse=" "),
  paste(amazon_reviews[emotions$joy > 0], collapse=" "),
  paste(amazon_reviews[emotions$sadness > 0], collapse=" "),
  paste(amazon_reviews[emotions$surprise > 0], collapse=" "),
  paste(amazon_reviews[emotions$trust > 0], collapse=" "),
  paste(amazon_reviews[emotions$positive > 0], collapse=" "),
  paste(amazon_reviews[emotions$negative > 0], collapse=" ")
)
wordcloud_tweet
# create corpus
